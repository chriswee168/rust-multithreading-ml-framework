...
{
    /// Remove an edge.
    pub fn remove_edge(&mut self, edge_id: String)
    {
        // Get edge arc and type.
        let (edge_type, edge) = 
            self.obtain_edge(edge_id.clone());
        ...
        if edge_type == "input" || edge_type == "output"
        {
            ...
            if edge_type == "input"
            {
                ...
                io_neuron.remove_backward_edge(edge_id.clone());
            ...
            else
            {
                ...
                io_neuron.remove_forward_edge(edge_id.clone());
