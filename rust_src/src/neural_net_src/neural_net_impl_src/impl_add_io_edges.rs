...
use crate::neural_net_src::{edge_src::{element_mutexed_vec::ElementMutexedVec, input_edge::InputEdge, output_edge::OutputEdge}, neural_net::NeuralNet, neuron_src::core_deps::NeuronTrait, rand_id_gen::rand_id_gen, types_aliases::{ArcEdgeTrait, ArcNeuronTrait}};
        ...
        let input_edge: ArcEdgeTrait = 
            ArcEdgeTrait::new(
                Mutex::new(
                    Box::new(
                        InputEdge::new(
                            input_array_idx, input_neuron.clone(), 
                            input_mutexed_vec, edge_weight_range
                        )
                    )
                )
            );
                ...
                Mutex::new(
                    Box::new(
                        OutputEdge::new(
                            output_neuron.clone(), 
                            output_array_idx,
                            output_mutexed_vec, edge_weight_range
                        )
                    )
                )
            );
