...
impl NeuralNet
{
    ...
    pub fn obtain_edge(&self, edge_id: String) -> Option<ArcEdgeTrait>
    {
        if self.input_edges.get(&edge_id).is_some()
        {
            return Some(self.input_edges.get(&edge_id).unwrap().clone());
        }
        else if self.hidden_edges.get(&edge_id).is_some() 
        {
            return Some(self.hidden_edges.get(&edge_id).unwrap().clone());
        }
        else if self.output_edges.get(&edge_id).is_some()
        {
            return Some(self.output_edges.get(&edge_id).unwrap().clone());
        }
        else
        {
            None
